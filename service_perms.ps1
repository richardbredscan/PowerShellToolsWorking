...
ForEach ($Item in $Items){
	...
	($exe_perm = ($exe_perm | Select-String "(?smi)(Path\s+:.*?(BUILTIN|AUTHENTICATED)\\\S+\s+Allow\s+FullControl)" -AllMatches| %{$_.Matches} | %{$_.Value})) | Out-Null
...
<# check registry perms #>
	...
	($reg_perm = ($reg_perm | Select-String "(?smi)(Path\s+:.*?(BUILTIN|AUTHENTICATED)\\\S+\s+Allow\s+FullControl)" -AllMatches| %{$_.Matches} | %{$_.Value})) | Out-Null
